...
1.  You can change various settings and options for your mail system in
    ...
    You are welcome to change settings as necessary or just for fun. Go ahead
...
yourself (your Mudd email or other email account). Make sure you include a 
...
The lines in parentheses (from, to, cc, and subject) are optional and just
change how the message looks when it is received. Note that you may need to use
sudo to send emails. Avoid blank lines between lines because postfix gets
confused.
...
will us mutt. Mutt is easier to install simply because it has a nicely 
...
1.  To start mutt, simply type mutt into the command line:
    ...
    Mutt may offer to create a mailbox for you. If it doesn't and gives 
    you the error that .maildir is not a maibox, go to the .maildir directory 
...
Set up a mail alias and add your email to it.
